...
#ifdef CONFIG_RTC_ALARM
		if (alarm_cb) {
			data->alarm_cb(dev, RTC_EVENT_ALARM_IRQ, alarm_cb_data);
...
static int rtc_renesas_ra_set_time(const struct device *dev, const struct rtc_time *timeptr)
{
	...
	/* Day of the year, Daylight saving time flag, nano seconds are not support by the hardware
	 */
...
#define RTC_RCR4_RCKSEL_LOCO_CLOCK_MASK BIT(0)
#define RTC_RADJ_ADJUST_VALUE_MASK      (0x3F)
...
static int rtc_renesas_ra_get_calibration(const struct device *dev, int32_t *calibration)
{
	ARG_UNUSED(dev);

	uint8_t adjust_value;
	int32_t calibration_temp = 0;

	/* Calibration is not available while using the LOCO clock */
	if (R_RTC->RCR4_b.RCKSEL & RTC_RCR4_RCKSEL_LOCO_CLOCK_MASK) {
		LOG_DBG("Calibration is not available while using the LOCO clock");
		return -ENOTSUP;
	}

	adjust_value = R_RTC->RADJ & RTC_RADJ_ADJUST_VALUE_MASK;

	if (R_RTC->RCR2_b.AADJP == RTC_ERROR_ADJUSTMENT_PERIOD_1_MINUTE) {
		calibration_temp = CYCLES_TO_PPB_EACH_1_MINUTE(adjust_value);
	} else {
		calibration_temp = CYCLES_TO_PPB_EACH_10_SECOND(adjust_value);
	}

	if (R_RTC->RADJ_b.PMADJ == RTC_ERROR_ADJUSTMENT_SUBTRACT_PRESCALER) {
		calibration_temp = -calibration_temp;
	}

	*calibration = calibration_temp;
...
static int rtc_renesas_ra_set_calibration(const struct device *dev, int32_t calibration)
{
	...
	if (calibration == 0) {
		...
		goto set_calibaration;
	}

	/* Calculate number of cycles to adjust from ppb */
	switch (data->fsp_err_cfg.adjustment_period) {
	case RTC_ERROR_ADJUSTMENT_PERIOD_10_SECOND:
		adjustment_cycles = PPB_TO_CYCLES_PER_10_SECOND(abs(calibration));
		break;
	case RTC_ERROR_ADJUSTMENT_PERIOD_1_MINUTE:
		adjustment_cycles = PPB_TO_CYCLES_PER_1_MINUTE(abs(calibration));
		break;
	default:
		LOG_ERR("Invalid adjustment period: %u", data->fsp_err_cfg.adjustment_period);
		return -EINVAL;
	}

	if (adjustment_cycles > RTC_RENESAS_RA_MAX_ERROR_ADJUSTMENT_VALUE) {
		LOG_ERR("Calibration out of HW range");
		return -EINVAL;
	}

	data->fsp_err_cfg.adjustment_type = (calibration > 0)
...
static DEVICE_API(rtc, rtc_renesas_ra_driver_api) = {
	...
	.get_calibration = rtc_renesas_ra_get_calibration,
#endif /* CONFIG_RTC_CALIBRATION */
};
...
#define RTC_RENESAS_CALIBRATION_MODE_GET(index)                                                    \
	COND_CODE_1(CONFIG_RTC_CALIBRATION, (RTC_ERROR_ADJUSTMENT_MODE_AUTOMATIC),		   \
		    (RTC_ERROR_ADJUSTMENT_MODE_MANUAL))

#define RTC_RENESAS_CALIBRATION_PERIOD_GET(index)                                                  \
	COND_CODE_1(CONFIG_RTC_CALIBRATION,                                                        \
		(DT_INST_ENUM_IDX(index, error_adjustment_period)),                                \
...
#define RTC_RENESAS_RA_INIT(index)                                                                 \
	...
	static struct rtc_renesas_ra_data rtc_renesas_ra_data_##index = {                          \
		.fsp_err_cfg =                                                                     \
			{                                                                          \
				.adjustment_mode = RTC_RENESAS_CALIBRATION_MODE_GET(index),        \
				.adjustment_period = RTC_RENESAS_CALIBRATION_PERIOD_GET(index),    \
		...
		.fsp_cfg =                                                                         \
			{                                                                          \
				...
				.carry_irq = RTC_RENESAS_RA_IRQ_GET(index, cup, priority),         \
